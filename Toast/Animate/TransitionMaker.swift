...
internal class TransitionMaker<Layer, Value> where Layer : CALayer, Value : RawRepresentable, Value.RawValue == String {
    internal unowned let maker:AnimationsMaker<Layer>
    internal let style:TransitionStyle
    internal init(maker:AnimationsMaker<Layer>, style:TransitionStyle) {
        self.maker = maker
        self.style = style
    }
    
    @discardableResult
    internal func animate(from direction:Value, duration:TimeInterval) -> AnimationTransition<Value> {
...
internal enum TransitionNone: String {
    case none = ""
}


internal enum TransitionVertical: String {
    case top    = "fromTop"
    case bottom = "fromBottom"
}

internal enum TransitionHorizontal: String {
    case left   = "fromLeft"
    case right  = "fromRight"
}

internal enum TransitionDirection: String {
    case none   = ""
    case left   = "fromLeft"
    case right  = "fromRight"
    case top    = "fromTop"
...
internal enum TransitionStyle: String {
